...
class SichuanOilSource:
    ...
    def get_today_listing(self, today: date) -> OilListingItem | None:
        response = retry_http_call(
            lambda: httpx.get(SC_FGW_NEWS_LIST_URL, timeout=self.timeout),
    ...
    def get_attachment(self, page_url: str) -> OilAttachment | None:
        response = retry_http_call(
            lambda: httpx.get(page_url, timeout=self.timeout),
    ...
    def parse_docx_prices(self, attachment_url: str) -> list[str]:
        response = retry_http_call(
            lambda: httpx.get(attachment_url, timeout=self.timeout),
...
class OilPriceJob:
    ...
    def run(self, today: date | None = None) -> JobResult:
        ...
        if self.state_store.has_sent("oil_price", day_text):
        ...
        self.state_store.mark_sent(
            "oil_price",
            day_text,
            {"title": listing.title, "page_url": listing.page_url},
        )
